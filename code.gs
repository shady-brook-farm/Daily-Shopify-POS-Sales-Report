...
function processCSVData() {
  SpreadsheetApp.getActiveSpreadsheet().toast(
    "Processing data...\n\n Please wait for success message.",
    "Status",
    5
  );
  ...
  try {
    ...
    if (!data || data.length < 2) {
      throw new Error(
        "Insufficient data: Need at least header row and one data row"
      );
    }
    ...
    if (colLocation === -1 || colDate === -1 || colQty === -1) {
      throw new Error(
        "Missing required headers: POS location name, Day, Net items sold."
      );
    }
    ...
    rows.forEach(function (row) {
      ...
      var product = String(
        row[colVariant] || row[colProduct] || "Unknown"
      ).trim();
      ...
      pivot[location][product][dateLabel] =
        (pivot[location][product][dateLabel] || 0) + qty;
    ...
    if (output.length > 1) {
      var dataRange = newSheet.getRange(
        2,
        1,
        output.length - 1,
        headerRow.length
      ...
      for (var r = 2; r <= output.length; r++) {
        ...
        if (
          !currentRow ||
          currentRow.every(function (cell) {
            return cell === "";
          })
        ) {
          continue;
        ...
        newSheet
          .getRange(r, 1, 1, headerRow.length)
          .setBackgrounds([fullRowColors]);
    ...
    newSheet
      .getRange(summaryRow, summaryCol)
      .setValue("SUMMARY")
      .setFontWeight("bold")
      .setFontSize(14)
      .setBackground("#d9d9d9");
    ...
    if (colSales !== -1) {
      var totalSales = Object.values(productTotals).reduce(function (
        acc,
        product
      ) {
      ...
      newSheet
        ...
        .setFontWeight("bold");
      newSheet
        .getRange(summaryRow, summaryCol + 1)
    ...
    newSheet
      .getRange(breakdownStartRow, breakdownCol)
    ...
    newSheet
      .getRange(breakdownStartRow, breakdownCol, 1, summaryHeaders.length)
    ...
    productArray.forEach(function (product, index) {
      ...
      // Write row to sheet
      newSheet
        .getRange(breakdownStartRow + 1 + index, breakdownCol, 1, row.length)
        .setValues([row]);
    });
    ...
    SpreadsheetApp.getUi().alert("Error Processing Data: " + error.message);
  }
}
